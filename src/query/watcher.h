...
struct _GsqWatcherClass {
	...
	void (*name_changed) (GsqWatcher *watcher);
	void (*gameid_changed) (GsqWatcher *watcher);
	void (*map_changed) (GsqWatcher *watcher);
};


GType gsq_watcher_get_type (void) G_GNUC_CONST;
GsqWatcher* gsq_watcher_new (const gchar *address);


void gsq_watcher_set_offline_time (GsqWatcher *watcher, guint64 time);
guint64 gsq_watcher_get_offline_time (GsqWatcher *watcher);
gboolean gsq_watcher_is_online (GsqWatcher *watcher);
